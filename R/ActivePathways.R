...
ActivePathways_method <- function(multiGene,
    ...
    em <- ActivePathways::ActivePathways(scores = scores, gmt = gmts2,
        correction.method = "none", significant = 1,
        background = background, merge.method = merge.method, ...)
    ...
    nTermGene <- vapply(gmts[em$term.id], nrow, FUN.VALUE = 1)
    N <- length(unique(TERM2GENE$gene))
    BgRatio <- apply(data.frame(a=nTermGene, b=N), 1, function(x)
                     paste(x[1], "/", x[2], sep="", collapse="")
                     )
    p.adj <- p.adjust(em$adjusted.p.val, method=pAdjustMethod)
    qobj <- tryCatch(qvalue(p=em$adjusted.p.val, lambda=0.05, pi0.method="bootstrap"),
        error=function(e) NULL)
    geneID <- vapply(em$overlap, function(i) paste(i, collapse="/"), FUN.VALUE = "1")
    result <- data.frame(ID          = em$term.id,
                         Description = em$term.name,
                         GeneRatio   = GeneRatio,
                         BgRatio     = BgRatio,
                         pvalue      = em$adjusted.p.val,
                         p.adjust    = p.adj,
                         qvalue      = qobj$qvalue,
                         geneID      = geneID,
                         Count       = Count)
